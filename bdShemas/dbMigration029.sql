...
CREATE
PROCEDURE loadPgByLabInternal(IN p_lab int, IN p_state int)
  READS SQL DATA
BEGIN
  -- create temp
  ...
  INSERT INTO tmp_pgid (id)
    SELECT t.id
      FROM (SELECT pg.id, IFNULL(IFNULL(MAX(IF(s.type = 7 AND bs.synonym_type = 1, bpt.lab_type, NULL)), MAX(IF(bs.synonym_type = 0, bpt.lab_type, NULL))), 0) lab_type
          FROM lab l
            INNER JOIN lab_print_code lpc ON l.src_type = lpc.src_type
...
BEGIN
  ...
  -- load pg by lab
  CALL loadPgByLabInternal(p_lab, 200);
  ...
  THEN
    ...
    INSERT INTO prn_queue (strategy, is_active, created, label, has_sub, lab)
      VALUES (1, 1, NOW(), CONCAT_WS(';', IF(p_reprint, 'Перепечатка', NULL), vPaper, p_width, IF(p_booksonly,'Книги',NULL)), 0, p_lab);
...
BEGIN
  ...
  THEN
    ...
    -- create queue, strategy now is strategy type !!!! 
    INSERT INTO prn_queue (strategy, is_active, created, label, has_sub, lab)
      VALUES (3, 1, NOW(), CONCAT_WS(';', IF(p_reprint, 'Перепечатка', NULL), p_alias, vBookPart, p_sheet_num, IF(p_booksonly,'Книги',NULL)), 0, p_lab);
    SET vQueueId = LAST_INSERT_ID();
