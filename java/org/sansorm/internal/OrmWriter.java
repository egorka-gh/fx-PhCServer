...
public class OrmWriter extends OrmBase
{
    ...
    public static <T> T insertObject(Connection connection, T target, String[] columnNames) throws SQLException{
    	if(columnNames==null || columnNames.length==0) return insertObject(connection, target);
        Class<?> clazz = target.getClass();
        Introspected introspected = Introspector.getIntrospected(clazz);
    ...
    public static <T> T updateObject(Connection connection, T target, String[] columnNames) throws SQLException{
    	if(columnNames==null || columnNames.length==0) return updateObject(connection, target);
        Class<?> clazz = target.getClass();
        Introspected introspected = Introspector.getIntrospected(clazz);
    ...
    /*
    public static int executeInsert(Connection connection, String sql, Object... args) throws SQLException{
        PreparedStatement stmt = null;
        try
        {
            stmt = connection.prepareStatement(sql);
